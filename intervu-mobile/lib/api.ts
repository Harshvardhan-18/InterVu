/**
 * InterVu API client — React Native version
 * Mirrors frontend/lib/api.ts exactly.
 * Auth storage uses expo-secure-store instead of localStorage.
 */
...
const AUTH_KEY = 'intervu_user';

export const auth = {
  save: async (data: LoginResponse): Promise<void> => {
    await SecureStore.setItemAsync(AUTH_KEY, JSON.stringify(data));
  },

  load: async (): Promise<LoginResponse | null> => {
